...
print('Detta är den senaste verisionen')
...
function GenerateUniquePhoneNumber()
  ...
  while not foundNumber do
    ...
    phoneNumber = math.random(100000, 99999999)
...
AddEventHandler('esx_phone:checkItem', function()
  ...
  local item    = xPlayer.getInventoryItem('esx_phone3').count
  if item > 0 then
    TriggerClientEvent('esx_phone:openPhone', _source)
	else
		TriggerClientEvent("pNotify:SendNotification", source,{
            text = "Du har ingen telefon!",
            type = "error",
            timeout = (2000),
            layout = "Bottomcenter",
            queue = "global"
...
AddEventHandler('esx_phone:startCall', function(number)
  ...
  if foundPlayer ~= false then
    ...
    TriggerClientEvent('esx_phone:endCall', _source, 'Kontakten är offline')
  end
    ...
    function(result)
      ...
      MySQL.Async.fetchAll(
        ...
        function(result2)
          ...
          local card = xPlayer.getInventoryItem('encrypted_simcard')['count']
          if card > 0 then
              encryptingCard = true
          end

          TriggerClientEvent('esx_phone:loaded', source, phoneNumber, contacts, encryptingCard, xPlayer.identifier)
...
AddEventHandler('esx_phone:reload', function(phoneNumber)
  ...
  local encryptingCard = xPlayer.getInventoryItem('encrypted_simcard')
  if encryptingCard then
      if encryptingCard['count'] > 0 then
          encryptingCard = true
      end
  end
          ...
          if hasAlreadyAdded then
            ...
            MySQL.Async.execute(
              ...
              function(rowsChanged)
                ...
                TriggerClientEvent('esx:showNotification', _source, 'Tillagd kontakt')
...
AddEventHandler('esx_phone:billCall', function(duration)
  ...
  TriggerClientEvent('esx:showNotification', _source, 'Ditt samtal kostar dig SEK' .. amount .. ' ')
...
AddEventHandler('esx_phone:bankTransfer', function(target, amount)
  ...
  if amount > 0 and amount <= xPlayer.getAccount('bank').money then
	...
	TriggerClientEvent('esx:showNotification', _source, 'Du har tagit bort ' .. amount .. ' till ' .. targetXPlayer.name)

  else
	TriggerClientEvent('esx:showNotification', _source, 'Ogiltigt belopp')
  end
...
AddEventHandler('esx_phone:tryOpenPhone', function()
	...
	if xPlayer ~= nil then
		if xPlayer.getInventoryItem('phone').count >= 1 then
			print('openPhone-serv')
			TriggerClientEvent('esx_phone:openPhone', source)
		else
			sendNotification(source, 'Du har ingen telefon', 'error', 2500)
		end
	end
end)
...
ESX.RegisterServerCallback('esx_phone3:getItemAmount', function(source, cb, item)
    local xPlayer = ESX.GetPlayerFromId(source)
